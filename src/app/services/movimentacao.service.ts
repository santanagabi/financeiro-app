...
/**
 * @Injectable({ providedIn: 'root' }) é o equivalente a criar uma store no Pinia
...
export class MovimentacaoService {
  ...
  async carregarDados(): Promise<void> {
    ...
    // Primeiro acesso: busca o JSON mock via HttpClient (igual um fetch/axios.get)
  ...
  async salvar(nova: NovaMovimentacao): Promise<{ ok: boolean; mensagem: string }> {
  ...
  private validar(nova: NovaMovimentacao): string | null {
    if (!nova.data || !nova.tipo || !nova.categoria || !nova.descricao || nova.valor == null) {
